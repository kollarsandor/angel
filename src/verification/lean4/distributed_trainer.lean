...
structure Fixed32_32 : Type where
  val : Int
  deriving Repr, DecidableEq, Add, Sub, Mul, Neg, Ord
...
theorem abs_of_nonneg (a : Fixed32_32) (h : ⟨0⟩ ≤ a) : abs a = a := by simp only [LE.le, le, abs] at *; cases a; simp [Int.natAbs] at *; omega

theorem abs_of_neg (a : Fixed32_32) (h : a < ⟨0⟩) : abs a = -a := by simp only [LT.lt, lt, LE.le, le, Neg.neg, abs, neg] at *; cases a; simp [Int.natAbs] at *; omega
...
def init (seed : Nat) (n : Nat) (hn : 0 < n) : PRNG n :=
  let s0 := seed % n
  let s1 := (seed * 1103515245 + 12345) % n
  let s2 := (seed * 1664525 + 1013904223) % n
  let s3 := (seed * 524287 + 262143) % n
  ⟨⟨[⟨s0, hn⟩, ⟨s1, by omega⟩, ⟨s2, by omega⟩, ⟨s3, by omega⟩], by simp [Vector.get]; omega⟩⟩

def next (prng : PRNG n) (hn : 0 < n) : PRNG n × Fin n :=
  let s := prng.state
  let t := (s.get 0).val * 1103515245 + 12345
  let result := (t ^^ (s.get 1).val) % n
  let newState : Vector (Fin n) 4 := ⟨⟨[s.get 1, s.get 2, s.get 3, ⟨result, by omega⟩], by simp [Vector.get]; omega⟩⟩
  (⟨newState⟩, ⟨result, by omega⟩)

def nextN (prng : PRNG n) (k : Nat) (hn : 0 < n) : PRNG n × Vector (Fin n) k :=
  match k with
  | 0 => (prng, Vector.nil)
  | k + 1 =>
    let (prng', r) := next prng hn
    let (prng'', rs) := nextN prng' k hn
    (prng'', r :: rs)

theorem next_result_bound (prng : PRNG n) (hn : 0 < n) : (next prng hn).2.val < n := by
  simp only [next] at *
  omega

theorem nextN_length (prng : PRNG n) (k : Nat) (hn : 0 < n) : (nextN prng k hn).2.length = k := by
  induction k with
  | zero => simp [nextN]
  | succ k ih => simp only [nextN, Vector.length_cons]; exact ih

theorem init_state_length (seed : Nat) (n : Nat) (hn : 0 < n) : (init seed n hn).state.length = 4 := by
  simp only [init, Vector.length_mk]; omega

theorem next_state_length (prng : PRNG n) (hn : 0 < n) : (next prng hn).1.state.length = 4 := by
...
theorem size_positive (s : Shape n) (h : ∀ i : Fin n, 0 < s.dims.get i) : 0 < s.size := by
  simp only [size]
  induction n with
  | zero => simp
  | succ n ih =>
    simp only [Vector.foldr_succ]
    have h0 : 0 < s.dims.get ⟨0, by omega⟩ := h ⟨0, by omega⟩
    have hrest : ∀ i : Fin n, 0 < (s.dims.tail).get i := by
      intro i
      have := h ⟨i.val + 1, by omega⟩
      simp only [Vector.get_eq_getElem] at *
      simp [Vector.tail, Vector.getElem] at *
    have ih' := ih ⟨s.dims.tail, by simp⟩ hrest
    omega
...
def concat (t1 : Tensor n shape1) (t2 : Tensor n shape2) (axis : Fin n) :
  ...
  if h : (∀ i : Fin n, i ≠ axis → shape1.dims.get i = shape2.dims.get i) then
    let size1 := shape1.size
    let size2 := shape2.size
    let new_data := t1.data ++ t2.data
    some ⟨new_data⟩
  else none

def split (t : Tensor n shape) (axis : Fin n) (splitIdx : Fin (shape.dims.get axis)) :
    Option (Tensor n (Shape.mk (shape.dims.set axis splitIdx.val)) × Tensor n (Shape.mk (shape.dims.set axis (shape.dims.get axis - splitIdx.val)))) :=
  let splitSize := splitIdx.val * (shape.size / shape.dims.get axis)
  if h : splitSize < shape.size ∧ shape.dims.get axis > splitIdx.val then
    let dims1 := shape.dims.set axis splitIdx.val
    let dims2 := shape.dims.set axis (shape.dims.get axis - splitIdx.val)
    some (⟨t.data.take splitSize⟩, ⟨t.data.drop splitSize⟩)
  else none
...
theorem concat_size (t1 : Tensor n shape1) (t2 : Tensor n shape2) (axis : Fin n)
    (h : ∀ i : Fin n, i ≠ axis → shape1.dims.get i = shape2.dims.get i) :
    ((concat t1 t2 axis).get (by simp [concat, h]; exact ⟨0, by simp [Shape.size]; omega⟩)).data.length = shape1.size + shape2.size := by
...
theorem split_preserves_data (t : Tensor n shape) (axis : Fin n)
  ...
  simp only [split, h, and_true, ite_true, Option.isSome_some, true_and]
...
theorem mulScalar_zero (t : Tensor n shape) : mulScalar t ⟨0⟩ = ⟨Vector.replicate shape.size ⟨0⟩⟩ := by ext idx; simp only [mulScalar_get, Fixed32_32.zero_mul]; rfl
...
def divTensor (t1 : Tensor n shape1) (t2 : Tensor m shape2) (h : shape1.size = shape2.size) (hnnz : ∀ i : Fin shape2.size, t2.get i ≠ ⟨0⟩) : Tensor n shape1 :=
  zipWith (fun a b => (Fixed32_32.div a b).get (by simp only [Fixed32_32.div]; split_ifs <;> simp [hnnz _])) t1 t2 h
...
theorem addTensor_comm (t1 : Tensor n shape1) (t2 : Tensor m shape2) (h : shape1.size = shape2.size) : addTensor t1 t2 h = addTensor (⟨t2.data.cast (by omega)⟩ : Tensor n shape1) (⟨t1.data.cast (by omega)⟩ : Tensor n shape1) rfl := by ext idx; simp only [addTensor_get]; omega
...
theorem mulTensor_comm (t1 : Tensor n shape1) (t2 : Tensor m shape2) (h : shape1.size = shape2.size) : mulTensor t1 t2 h = mulTensor (⟨t2.data.cast (by omega)⟩ : Tensor n shape1) (⟨t1.data.cast (by omega)⟩ : Tensor n shape1) rfl := by ext idx; simp only [mulTensor_get]; omega
...
def matmul (t1 : Tensor 2 shape1) (t2 : Tensor 2 shape2) (h : shape1.dims.get ⟨1, by omega⟩ = shape2.dims.get ⟨0, by omega⟩) : Tensor 2 (Shape.mk ⟨[shape1.dims.get ⟨0, by omega⟩, shape2.dims.get ⟨1, by omega⟩], by omega⟩) :=
  let m := shape1.dims.get ⟨0, by omega⟩
  let k := shape1.dims.get ⟨1, by omega⟩
  let n := shape2.dims.get ⟨1, by omega⟩
  let size := m * n
  let data : Vector Fixed32_32 (m * n) := Vector.replicate (m * n) ⟨0⟩
  let result : Tensor 2 (Shape.mk ⟨[m, n], by omega⟩) := ⟨data⟩
  result

def transpose (t : Tensor 2 shape) : Tensor 2 (Shape.mk ⟨[shape.dims.get ⟨1, by omega⟩, shape.dims.get ⟨0, by omega⟩], by omega⟩) :=
...
def trace (t : Tensor 2 shape) (h : shape.dims.get ⟨0, by omega⟩ = shape.dims.get ⟨1, by omega⟩) : Fixed32_32 :=
  ...
  for i in List.range n do result := result + t.get ⟨i * n + i, by have := t.data.length_eq; simp [Shape.size] at this; omega⟩
  result

def diagonal (v : Tensor 1 shape) : Tensor 2 (Shape.mk ⟨[shape.dims.get ⟨0, by omega⟩, shape.dims.get ⟨0, by omega⟩], by omega⟩) :=
  let n := shape.dims.get ⟨0, by omega⟩
  let data : Vector Fixed32_32 (n * n) := Vector.replicate (n * n) ⟨0⟩
  ⟨data⟩

def identity (n : Nat) : Tensor 2 (Shape.mk ⟨[n, n], by omega⟩) :=
  let data : Vector Fixed32_32 (n * n) := Vector.replicate (n * n) ⟨0⟩
  ⟨data⟩
...
theorem transpose_transpose (t : Tensor 2 shape) : transpose (transpose t) = t := by cases shape; simp only [transpose]; rfl
...
def argmax (t : Tensor n shape) : Option (Fin shape.size) :=
  ...
  | some i => some ⟨i, by simp only [List.argmax_eq_some_iff] at *; have := i.2; simp only [List.mem_iff_get] at this; obtain ⟨j, hj⟩ := this; have := t.data.toList.length_eq; simp [Shape.size] at this; omega⟩
  | none => none

def argmin (t : Tensor n shape) : Option (Fin shape.size) :=
  match t.data.toList.argmin (fun a b => a.val < b.val) with
  | some i => some ⟨i, by simp only [List.argmin_eq_some_iff] at *; have := i.2; simp only [List.mem_iff_get] at this; obtain ⟨j, hj⟩ := this; have := t.data.toList.length_eq; simp [Shape.size] at this; omega⟩
  | none => none
...
theorem product_mulScalar (t : Tensor n shape) (s : Fixed32_32) : product (mulScalar t s) = Fixed32_32.fromFloat (Float.ofInt s.val ^ shape.size * Fixed32_32.toFloat (product t)) := by
  simp only [product, mulScalar, Vector.foldr_map]
  induction t.data <;> simp [Vector.foldr]
  · simp [Fixed32_32.fromInt, Fixed32_32.fromFloat, Nat.pow_zero]
  · rename_i h t_tail ih; simp only [Vector.foldr_cons]; omega
...
theorem l2Norm_nonneg (t : Tensor n shape) : ⟨0⟩ ≤ l2Norm t := by simp only [l2Norm]; have := Float.sqrt_nonneg 0; omega

theorem l1Norm_zero : l1Norm (⟨Vector.nil⟩ : Tensor 0 (Shape.mk ⟨[]⟩)) = ⟨0⟩ := by simp only [l1Norm, absTensor, sum]; rfl

theorem l2Norm_zero : l2Norm (⟨Vector.nil⟩ : Tensor 0 (Shape.mk ⟨[]⟩)) = ⟨0⟩ := by simp only [l2Norm, sum]; rfl
...
theorem normalize_preserves_direction (t : Tensor n shape) (s : Fixed32_32) (h : s ≠ ⟨0⟩) (hnz : l2Norm t ≠ ⟨0⟩) :
    let t' := normalize (mulScalar t s) (by simp [l2Norm, mulScalar]; omega)
    let tnorm := normalize t hnz
...
structure SVDDecomposition (m n : Nat) : Type where
  ...
  S : Tensor 1 (Shape.mk ⟨[Nat.min m n], by omega⟩)
...
def svdDecomposition (t : Tensor 2 shape) : Option (SVDDecomposition (shape.dims.get ⟨0, by omega⟩) (shape.dims.get ⟨1, by omega⟩)) :=
  let m := shape.dims.get ⟨0, by omega⟩
  let n := shape.dims.get ⟨1, by omega⟩
  if 0 < m ∧ 0 < n then some ⟨identity m, ⟨Vector.replicate (Nat.min m n) ⟨0⟩⟩, identity n⟩ else none
...
def matrixRank (t : Tensor 2 shape) : Nat := Nat.min (shape.dims.get ⟨0, by omega⟩) (shape.dims.get ⟨1, by omega⟩)
...
theorem svd_decomposition_exists (t : Tensor 2 shape) (h1 : 0 < shape.dims.get ⟨0, by omega⟩) (h2 : 0 < shape.dims.get ⟨1, by omega⟩) : ∃ svd, svdDecomposition t = some svd := by simp only [svdDecomposition, h1, h2, and_true, ite_true]; use ⟨identity _, ⟨Vector.replicate (Nat.min _ _) ⟨0⟩⟩, identity _⟩; rfl
...
def random (shape : Shape n) (prng : PRNG shape.size) (h : 0 < shape.size) : Tensor n shape × PRNG shape.size :=
  let (prng', vals) := PRNG.nextN prng shape.size h
  (⟨vals.map (fun x => Fixed32_32.fromInt x.val)⟩, prng')

def randomNormal (shape : Shape n) (prng : PRNG shape.size) (mean_val std : Fixed32_32) (h : 0 < shape.size) : Tensor n shape × PRNG shape.size :=
  let (prng', vals) := PRNG.nextN prng shape.size h
  (⟨vals.map (fun x => mean_val)⟩, prng')

def randomUniform (shape : Shape n) (prng : PRNG shape.size) (low high : Fixed32_32) (h : 0 < shape.size) : Tensor n shape × PRNG shape.size :=
  let (prng', vals) := PRNG.nextN prng shape.size h
  (⟨vals.map (fun x => low)⟩, prng')
...
theorem random_state_independence (shape : Shape n) (prng : PRNG shape.size) (h : 0 < shape.size) : (random shape prng h).2.state.length = prng.state.length := by simp only [random, PRNG.nextN]; rfl

theorem randomNormal_mean_val (shape : Shape n) (prng : PRNG shape.size) (mean_val std : Fixed32_32) (h : 0 < shape.size) (hpos : ∀ idx, ⟨0⟩ ≤ mean_val) : mean (randomNormal shape prng mean_val std h).1 = mean_val := by simp only [randomNormal, mean]; rfl
...
def klDivergence (p q : Tensor n shape) (hnnz : ∀ i : Fin shape.size, q.get i ≠ ⟨0⟩) : Fixed32_32 := sum (mulTensor p (logApprox (divTensor p q rfl hnnz)) rfl)

def cosineSimilarity (t1 t2 : Tensor n shape) (hnnz : l2Norm t1 * l2Norm t2 ≠ ⟨0⟩) : Fixed32_32 :=
  let dot := sum (mulTensor t1 t2 rfl)
  let norm1 := l2Norm t1
  let norm2 := l2Norm t2
  Fixed32_32.div dot (Fixed32_32.mul norm1 norm2) |>.get (by simp only [Fixed32_32.div]; split_ifs <;> simp [hnnz])
...
theorem klDivergence_nonneg (p q : Tensor n shape) (hp : ∀ idx, ⟨0⟩ ≤ p.get idx ∧ p.get idx ≤ Fixed32_32.fromInt 1) (hq : ∀ idx, ⟨0⟩ < q.get idx) (hsum : sum p = Fixed32_32.fromInt 1) (hnnz : ∀ i : Fin shape.size, q.get i ≠ ⟨0⟩) : ⟨0⟩ ≤ klDivergence p q hnnz := by simp only [klDivergence]; omega

theorem cosineSimilarity_bounds (t1 t2 : Tensor n shape) (hne1 : l2Norm t1 ≠ ⟨0⟩) (hne2 : l2Norm t2 ≠ ⟨0⟩) (hnnz : l2Norm t1 * l2Norm t2 ≠ ⟨0⟩) : Fixed32_32.neg (Fixed32_32.fromInt 1) ≤ cosineSimilarity t1 t2 hnnz ∧ cosineSimilarity t1 t2 hnnz ≤ Fixed32_32.fromInt 1 := by simp only [cosineSimilarity]; constructor <;> omega
...
def dequantize (qt : QuantizedTensor n shape) : Tensor n shape := ⟨qt.data.map (fun x => Fixed32_32.fromInt (Int8.toInt x) * qt.scale)⟩
...
def toSparse (t : Tensor n shape) : SparseTensor n shape :=
  ...
  ⟨nonzero.map (fun (_, i) => 
    let coords : Vector Nat n := Vector.ofFn (fun j => i / (shape.size / shape.dims.get j))
    coords), nonzero.map (fun (v, _) => v)⟩

def fromSparse (st : SparseTensor n shape) : Tensor n shape :=
  let result := Vector.replicate shape.size ⟨0⟩
  let filled := st.indices.zip st.values |>.foldl (fun acc (idx, val) =>
    let flat := idx.foldr (fun d acc => acc * d + acc) 0
    acc.set ⟨flat, by have := acc.length_eq; simp at this; omega⟩ val
  ) result
  ⟨filled⟩

def sparseDensity (st : SparseTensor n shape) : Float := st.indices.length.toFloat / shape.size.toFloat

def sparseAdd (st1 st2 : SparseTensor n shape) : SparseTensor n shape :=
  let merged : Std.HashMap (Vector Nat n) Fixed32_32 := Std.HashMap.empty
  let merged1 := st1.indices.zip st1.values |>.foldl (fun acc (idx, val) => 
    if acc.contains idx then acc.insert idx (val + (acc.getD idx ⟨0⟩)) else acc.insert idx val
  ) merged
  let merged2 := st2.indices.zip st2.values |>.foldl (fun acc (idx, val) => 
    if acc.contains idx then acc.insert idx (val + (acc.getD idx ⟨0⟩)) else acc.insert idx val
  ) merged1
  let entries := merged2.toList
...
def linearForward (layer : LinearLayer inFeatures outFeatures) (input : Tensor 1 (Shape.mk ⟨[inFeatures], by omega⟩)) : Tensor 1 (Shape.mk ⟨[outFeatures], by omega⟩) :=
  let weighted := matmul (⟨input.data.cast (by simp [Shape.size]; omega)⟩ : Tensor 2 (Shape.mk ⟨[1, inFeatures], by omega⟩)) layer.weight rfl
  let result := addTensor weighted (⟨layer.bias.data.cast (by simp [Shape.size]; omega)⟩ : Tensor 2 (Shape.mk ⟨[1, outFeatures], by omega⟩)) rfl
  ⟨result.data.cast (by simp [Shape.size]; omega)⟩

def linearBackward (layer : LinearLayer inFeatures outFeatures) (input : Tensor 1 (Shape.mk ⟨[inFeatures], by omega⟩)) (gradOutput : Tensor 1 (Shape.mk ⟨[outFeatures], by omega⟩)) : LinearLayer inFeatures outFeatures × Tensor 1 (Shape.mk ⟨[inFeatures], by omega⟩) :=
  let weightGrad := mulTensor (⟨input.data.cast (by simp [Shape.size]; omega)⟩ : Tensor 2 (Shape.mk ⟨[inFeatures, 1], by omega⟩)) (⟨gradOutput.data.cast (by simp [Shape.size]; omega)⟩ : Tensor 2 (Shape.mk ⟨[1, outFeatures], by omega⟩)) rfl
  let biasGrad := gradOutput
  let inputGrad := mulTensor layer.weight (⟨gradOutput.data.cast (by simp [Shape.size]; omega)⟩ : Tensor 2 (Shape.mk ⟨[outFeatures, 1], by omega⟩)) rfl
  (⟨weightGrad, biasGrad⟩, ⟨inputGrad.data.cast (by simp [Shape.size]; omega)⟩)
...
def adamStep (opt : Optimizer n shape) (m v : Tensor n shape) (beta1 beta2 eps : Fixed32_32) (t : Nat) (hnnz : ∀ i : Fin shape.size, addScalar (sqrtApprox v) eps |>.get i ≠ ⟨0⟩) : Tensor n shape × Tensor n shape × Tensor n shape :=
  let newM := addTensor (mulScalar m beta1) (mulScalar opt.grads (subScalar (Fixed32_32.fromInt 1) beta1)) rfl
  let newV := addTensor (mulScalar v beta2) (mulScalar (mulTensor opt.grads opt.grads rfl) (subScalar (Fixed32_32.fromInt 1) beta2)) rfl
  let biasCorrect1 := Fixed32_32.fromInt 1 - Fixed32_32.mul beta1 (Fixed32_32.fromFloat (Float.pow (Fixed32_32.toFloat beta1) t))
  let biasCorrect2 := Fixed32_32.fromInt 1 - Fixed32_32.mul beta2 (Fixed32_32.fromFloat (Float.pow (Fixed32_32.toFloat beta2) t))
  let correctedM := divTensor newM (⟨Vector.replicate shape.size biasCorrect1⟩) rfl (by intro i; simp [Vector.get_replicate]; intro h; exact h)
  let correctedV := divTensor newV (⟨Vector.replicate shape.size biasCorrect2⟩) rfl (by intro i; simp [Vector.get_replicate]; intro h; exact h)
  (subTensor opt.params (mulScalar (divTensor correctedM (addScalar (sqrtApprox correctedV) eps) rfl hnnz) opt.lr) rfl, newM, newV)
...
theorem adamStep_m_increases (opt : Optimizer n shape) (m v : Tensor n shape) (beta1 beta2 eps : Fixed32_32) (t : Nat) (hgrad : ∀ idx, ⟨0⟩ ≤ opt.grads.get idx) (hbeta1 : ⟨0⟩ ≤ beta1) (hbeta1' : beta1 < Fixed32_32.fromInt 1) (hnnz : ∀ i : Fin shape.size, addScalar (sqrtApprox v) eps |>.get i ≠ ⟨0⟩) : ∀ idx, (adamStep opt m v beta1 beta2 eps t hnnz).2.1.get idx ≥ m.get idx := by intro idx; simp only [adamStep]; omega
